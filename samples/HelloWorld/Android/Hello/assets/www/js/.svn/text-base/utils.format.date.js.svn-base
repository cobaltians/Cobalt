...
utils.formatting.date={
	/*	STRING FORMATTING
			...
			1) call utils.formatting.date.enable()
			2) use like this :
				var now = new Date();
				now.format("mm/dd/yy");
				or
				utils.formatting.date.format(now, "dddd, mmmm dS, yyyy, h:MM:ss TT");

			see http://blog.stevenlevithan.com/archives/date-time-format for full info
			*** warning : changes ***
			month and day locale has been changed to french. masks too.
	 */
	enable:function(){
		var self=this;
		/*
		...
		self.dateFormat = function () {
			...
			return function (date, mask, utc) {
				var dF = utils.formatting.date.dateFormat;
