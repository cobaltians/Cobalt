...
const String NativeBridgeForm::kOpenExternalUrl = (L"openExternalUrl");
...
const String NativeBridgeForm::kUrl = (L"url");
...
void NativeBridgeForm::Init(Web *web)
{
	if(__ressourcePath == null)
	{
		__ressourcePath = L"file://" + Tizen::App::App::GetInstance()->GetAppResourcePath() + L"www/";
	}
...
NativeBridgeForm::TreatData(IJsonValue* pJSData)
{
	...
	else if (pJSStringType->CompareTo(kTypeLog) == 0)
	{
		...
		if (DEBUG) AppLogDebugTag("NATIVE BRIDGE", "JSLOG: %ls", pJSStringValue->GetPointer());
	}
	...
	else if (pJSStringType->CompareTo(kTypeEvent) == 0)
	{
		...
		else if (pJSStringName->CompareTo(kOpenExternalUrl) == 0)
		{
			pJSObjectData->GetValue(&kUrl, pJSValueValue);
			pJSStringValue = static_cast<JsonString*> (pJSValueValue);
			if (pJSStringValue != null)
			{
				String uri = pJSStringValue->GetPointer();
			...
			{
				AppLogDebug("Error on url");
			}
		}
		else
		{
			jsRequestHandled = false;
		}
	}
	...
	else if (pJSStringType->CompareTo(kTypeNavigation) == 0)
	{
		...
		else if (DEBUG) AppLogDebugTag("NATIVE BRIDGE", "TYPE NAVIGATION NAME : %ls", pJSStringNavigationType->GetPointer());
	}
	...
	else if (pJSStringType->CompareTo(kTypeCallback) == 0)
	{
		...
		if (pJSValueCallbackID != null) {
			...
			if (pJSStringCallbackID->CompareTo(kJSHandleBackButtonPressedResult) == 0)
			{
				...
				HandleBackButtonPressedResult(pJSBoolAllowedToGoBack->ToBool());
			}
			else
			{
				jsRequestHandled = false;
		...
		else
		{
			...
			HandleBackButtonPressedResult(pAllowedToGoBack->ToBool());
		}
	}
	else
	{
		if (DEBUG) AppLogDebugTag("NATIVE BRIDGE", "Unhandled nativeBridge type");
		jsRequestHandled = false;
	}

	return jsRequestHandled;
}
...
void
NativeBridgeForm::PushWebView(JsonString* pJSStringActivityId, JsonString* pJSStringPageNamed, bool isNativeBridgeView, SceneTransitionAnimationType animation, SceneHistoryOption history)
{
	SceneManager* pSceneManager = SceneManager::GetInstance();
	AppAssert(pSceneManager);
	SceneId currentSceneId = pSceneManager->GetCurrentSceneId();

	if (! currentSceneId.Equals(pJSStringActivityId->GetPointer(), true))
	{
		if (DEBUG) AppLogDebugTag("NATIVE BRIDGE", "NBFORM - PushWebView: Push!");

		if (! pSceneManager->IsSceneAlive(pJSStringActivityId->GetPointer()))
		{
			if (DEBUG) AppLogDebugTag("NATIVE BRIDGE", "NBFORM - PushWebView: Scene does not exist, creating it...");

			if (isNativeBridgeView) {
				nativeBridgeIsReady = false;
			}
		...
		{
			// TODO: Send refresh for xiti
			JsonObject* pJsonObj = new JsonObject();
			JsonString* pKTypeEvent = new JsonString(kTypeEvent);
			JsonString* pjSStringRefresh = new JsonString (L"refresh");
			String script;
			char* pComposeBuf = new char[500];

			pJsonObj->Construct();
			pJsonObj->Add(&kType, pKTypeEvent);
			pJsonObj->Add(&kName, pjSStringRefresh);
			JsonWriter::Compose(pJsonObj, pComposeBuf, 500);
			script = pComposeBuf;
			if (DEBUG) AppLogDebugTag("NATIVE BRIDGE", "script envoyé pour le refresh : %ls", script.GetPointer());
			ExecuteScriptInWebView(&script);
		}

		pSceneManager->GoForward(ForwardSceneTransition(pJSStringActivityId->GetPointer(), animation, history));
	}
	else
	{
		if (DEBUG) AppLogDebugTag("NATIVE BRIDGE", "NBFORM - PushWebView: You want to push current scene you idiot!");
	}
}
...
NativeBridgeForm::SendCallbackResponse(JsonString* pJSStringCallbackId, String* pParamsToSend)
{
	...
	{
		if (DEBUG) AppLogDebugTag("NATIVE BRIDGE", "E_INVALID_ARG in JsonWriter ");
	}
	else if (r == E_INVALID_DATA)
	{
		if (DEBUG) AppLogDebugTag("NATIVE BRIDGE", "E_INVALID_DATA in JsonWriter");
	}
}
...
void
NativeBridgeForm::SendEvent(JsonString* pName,JsonObject* pObject)
{
	char* pComposeBuf = new char[1000];
	...
	JsonString* pJSStringTypeEvent = new JsonString(kTypeEvent);
	pObject->Add(&kType, pJSStringTypeEvent);
	pObject->Add(&kName, pName);

	JsonWriter::Compose(pObject, pComposeBuf, 1000);
	String scriptToSend(pComposeBuf);
	if (DEBUG) AppLogDebugTag("NATIVE BRIDGE", "script event envoyé : %ls", scriptToSend.GetPointer());
	ExecuteScriptInWebView(&scriptToSend);
}
void
