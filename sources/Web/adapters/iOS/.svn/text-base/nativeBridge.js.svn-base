		...
		switch(dependency){
			case "storage":
				if ( ! window.utils && ! window.utils.storage){
					nativeBridge.log('WARNING : window.utils.storage is not set. it is required for navigate(dismiss) calls')
					return false;
...
nativeBridge.ios_adapter={
	...
	//localStorage stuff
	initStorage:function(){
		//on ios, just check if localStorage is here and enable it
...
nativeBridge.android_adapter={
    ...
    // handle callbacks sent by native side
    handleCallback:function(callback){
    	//nativeBridge.log('received a callback',false)
        switch(callback.callbackID){
            default:
			    nativeBridge.tryToCallCallback(callback)
		    break;
